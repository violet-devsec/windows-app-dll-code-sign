...
namespace CodeSignApp.Security
{
    public static class AuthenticodeVerifier
    {
        ...
        public static extern WinVerifyTrustResult WinVerifyTrust([In] IntPtr hwnd, 
                                                                 [In] [MarshalAs(UnmanagedType.LPStruct)] Guid pgActionID, 
                                                                 [In] WinTrustData pWVTData
                                                                 );
        public static bool VerifySignature(String file)
        {
            WinTrustData wtd = new WinTrustData(file);
            Guid guidAction = new Guid(WINTRUST_ACTION_GENERIC_VERIFY_V2);
            WinVerifyTrustResult wtResult = WinVerifyTrust(INVALID_HANDLE_VALUE, guidAction, wtd);
            bool result = (wtResult == WinVerifyTrustResult.Success);
            return result;
        }
    }
